...
.controller('storyViewerCtrl', function($localStorage, $scope, $stateParams, $state, storyData) {
  ...
  $scope.selectRandomNarr = function(narrList){
    let maxIndex = narrList.length - 1;
  ...
  $scope.setPageData = function(storyData){
    ...
    if ($scope.pageType === 'par'){
      for(let i=1; i<=storyData.perPage; i++){
        let randNarr = $scope.selectRandomNarr(storyData.text.narrators);
        let parNum = i * $scope.currPage;
        pageData.paragraphs.push(storyData.text.paragraphs[randNarr][parNum]);
      }
    } else if ($scope.pageType === 'tangent') {
      // fill with tangent data
      for(let par in storyData.text.tangents[$scope.currPage]){
